...
?>
    ...
    <table class="table table-hover table-bordered add border-radius">
        ...
        <tr>
            ...
            <?php
                    ...
                    while($rows = mysqli_fetch_assoc($run_sql)){ ?>
       <?php echo '<tr><td>' .$i. '</td>'; ?><?php if($rows['transaction'] === 'Credit Transaction'){ echo'<td><p style="color: green";>'.$rows['transaction']. '</p></td>';}else{echo'<td><p style="color: red";>'.$rows['transaction']. '</p>';}?> <?php echo' <td>'.$rows['amount'].'</td>'; ?><?php echo' <td>'.$rows['created_at'].'</td>'; ?> <?php echo' <td>'.$rows['description'].'</td>'; ?> <?php echo '<td ><button type="button" class="btn btn-success btn-sm">'.$rows['Status']. '</button></td></tr>';
                    $i++;
                    } }else{
    ...
    </table><div class="print text-right">
                        <button onclick="window.print()"><i class="fa fa-print" aria-hidden="true"></i></button></div>
</div>
