...
if(isset($_POST['int_submit'])) {
    ...
    $blocked_sql = "SELECT * FROM blocked WHERE user_id ='$_SESSION[id]'";
    $blocked_q = mysqli_query($conn, $blocked_sql);
    if(mysqli_num_rows($blocked_q) > 0) {
        $inter_sql = "SELECT * FROM int_transfer WHERE user_id ='$_SESSION[id]'";
        $inter_q = mysqli_query($conn, $inter_sql);
        if(mysqli_num_rows($inter_q) > 0) {
            $int_update = "UPDATE int_transfer SET bank_name='$bank_name', amount='$amount', b_name='$b_name', b_acct='$b_acct', b_country='$b_country', swift_code='$swift_code', routing_number='$routing_number', description='$description', imf='$imf', refrence='$refrence'  WHERE user_id = '$_SESSION[id]'";
            $sql_q = mysqli_query($conn, $int_update);
            $sql_sel_int = "SELECT * FROM int_transfer WHERE user_id = '$_SESSION[id]'"; //FOR USERS
            $result1 = mysqli_query($conn, $sql_sel_int);
        ...
        $upd_sql = "UPDATE users SET  amount='$amount2', am_updated= '$date' WHERE id = '$_SESSION[id]'";
    ...
    'X-Mailer: PHP/' . phpversion();
        ...
        $sql_query = mysqli_query($conn, $sel_sql);
...
<strong>Treansfer limit exceeded</strong></div><br>';
						...
						<div class="di" style="padding-left: 10px; padding-right: 10px; padding-bottom: 10px;">
							...
							<div style="text-align: end; margin: 10px; position: relative">
								<button onclick="move()" class="btn btn-sm btn-danger" form="myform"
									name="int_submit">Continue</button>
								</form>
