        ...
        if(mysqli_num_rows($sql_query) > 0) {
            ...
            while($row = mysqli_fetch_assoc($query_user)) {
                ...
                if($query_update_user) {
                    ...
                    $subject2 = "FRDB Transaction Notification [Reversed: ".$row['currency'] . $rows['amount'] . "]";
                    ...
                    $message .= "<h4 style='color:#071d49;'>Your payment was not approved
                </4>";
                    $message .= '<h1 style="color: green; font-size:18px;">' .$row['currency'] .$rows['amount'].'.00</h1>';
                    $message .= '<h3>Transaction Summary</h3>';
                    $message .= '<p><b>IBAN:</b> '.$newact.' </p><p><b>Account type:</b> '.$row['account'].'<br></p>';
                    ...
                    $message .= '<h4>Your balance at the time of this transaction is <strong>' .$currency . $row['amount'] .'.00</strong> Thank you for chosing FRDBank</h4>';
