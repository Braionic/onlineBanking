...
?>
        <?php //BLOCK FROM PROVIDE TABLE BEGINS
             if(isset($_POST["block_submit"])) {
                 ...
                 $ins_sql = "UPDATE users SET status='blocked' WHERE id = '$_GET[user_id]'";
                 $run_sql = mysqli_query($conn, $ins_sql);
                 echo '<p>User has been blocked</p>';
...
if(isset($_POST["unblock_submit"])) {
    $block = "active";
    $currentid = $_GET['person_id'];
    $ins_sql = "UPDATE users SET status='$block' WHERE id = '$_GET[user_id]'";
    $run_sql = mysqli_query($conn, $ins_sql);
    echo '<p>Account has been restored</p>';
    header('Location: user_profile.php?user_id='.$currentid);
    //DELETE FROM PROVIDE TABLE ENDS
}
?>
...
while($rows = mysqli_fetch_assoc($run_sql)) {
    ...
    echo '<div class="pgd">
                            ...
                            <!-- <h4>Bank: '.$rows['bank'].'</h4>-->
                             ...
                             <form method="POST" action="user_profile.php?user_id='.$rows['id'].'" enctype="multipart/form-data">
    ...
    if($rows['status'] == "active") {
        echo '<form method="POST" action="user_profile.php?user_id='.$rows['id'].'" enctype="multipart/form-data">
    <button name="block_submit" onClick="return confirm(\'Block User?\')" class="profilebtn btn-success">Block User</button>
</form></div>
                             </div><div class"images">';
    } else {
        echo '<form method="POST" action="user_profile.php?user_id='.$rows['id'].'" enctype="multipart/form-data">
    <button name="unblock_submit" onClick="return confirm(\'Unblock User?\')" class="profilebtn btn-success">Unblock User</button>
</form></div>
                             </div>
                             <div class"images">';
    }
...
while($fetch = mysqli_fetch_assoc($run_sql)) {
    if($fetch['image'] == '') {
        echo '<img src="https://i.ibb.co/18PZkVk/tymebank-thumbnail-05-1080x1080-1.jpg" alt="profile pic" style="max-width: 200px; max-height: 200px;">';
    } else {
        echo '<img src="../uploaded_img/'.$fetch['image'].'" alt="profile pic" style="max-width: 200px; max-height: 200px;  margin-left: auto; margin-right: auto; display: block">';
    }
    echo'
    <p class="text-center bd-warning" style="font-size: 16px; color: white; background-color: grey; padding: 3px; border-radius: 6px;">'.$fetch["status"].'</p>
         <div class="profilebtns">
